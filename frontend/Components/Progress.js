...
export async function Progress() {
    ...
    const lines = projects.slice(1).map((project, i) => {
        ...
        return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="#E535AB" stroke-width="4" stroke-linecap="round" />`;
    }).join('');
    ...
    const circles = projects.map((project, i) => {
        ...
        return `
        <circle r="6" cx="${x}" cy="${y}" fill="#E535AB" stroke="white" stroke-width="1.5" style="cursor:pointer;">
            <title>${label} - ${FormatBytes(project.amount)}</title>
        </circle>`;
    }).join('');
